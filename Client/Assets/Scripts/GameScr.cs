...
public class GameScr : mScreen, IChatable
{
	...
	public void resetButton()
	{
		...
		mSystem.endKey();
		if (Char.myCharz().cHP <= 0.0 || Char.myCharz().statusMe == 14 || Char.myCharz().statusMe == 5)
		{
	...
	public override void updateKey()
	{
		...
		else
		{
			...
			else if (Char.myCharz().statusMe == 3)
			{
				...
				if ((GameCanvas.keyHold[(!Main.isPC) ? 2 : 21] || ((GameCanvas.keyHold[1] || GameCanvas.keyHold[3]) && mobCapcha == null)) && Char.myCharz().canFly && Char.myCharz().cMP > 0.0 && Char.myCharz().cp1 < 8 && Char.myCharz().cvy > -4)
				{
					Char.myCharz().cp1++;
					Char.myCharz().cvy = -7;
				}
			...
			else if (Char.myCharz().statusMe == 4)
			{
				...
				if (GameCanvas.keyHold[(!Main.isPC) ? 2 : 21] && Char.myCharz().cMP > 0.0 && Char.myCharz().canFly)
				{
					isAutoPlay = false;
			...
			else if (Char.myCharz().statusMe == 10)
			{
				...
				if (Char.myCharz().canFly && Char.myCharz().cMP > 0.0)
				{
	...
	private void checkClickMoveTo(int xClick, int yClick, int index)
	{
		...
		clickOnTileTop = false;
		Char.myCharz().delayFall = 0;
		int num = ((!Char.myCharz().canFly || Char.myCharz().cMP <= 0.0) ? 1000 : 0);
	...
	private void autoPlay()
	{
		...
		if (Char.myCharz().cHP <= Char.myCharz().cHPFull * 20.0 / 100.0 || Char.myCharz().cMP <= Char.myCharz().cMPFull * 20.0 / 100.0)
		{
			doUseHP();
		}
		if (Char.myCharz().mobFocus == null || (Char.myCharz().mobFocus != null && Char.myCharz().mobFocus.isMobMe))
		{
			for (int k = 0; k < vMob.size(); k++)
			{
				...
				if (mob2.status != 0 && mob2.status != 1 && mob2.hp > 0.0 && !mob2.isMobMe)
				{
		...
		else if (Char.myCharz().mobFocus.hp <= 0.0 || Char.myCharz().mobFocus.status == 1 || Char.myCharz().mobFocus.status == 0)
		{
			Char.myCharz().mobFocus = null;
		}
		...
		if (GameCanvas.isTouch)
		{
			for (int l = 0; l < onScreenSkill.Length; l++)
			{
				...
				double num = ((onScreenSkill[l].template.manaUseType == 2) ? 1.0 : ((onScreenSkill[l].template.manaUseType == 1) ? ((double)onScreenSkill[l].manaUse * Char.myCharz().cMPFull / 100.0) : ((double)onScreenSkill[l].manaUse)));
		...
		for (int m = 0; m < keySkill.Length; m++)
		{
			...
			double num2 = ((keySkill[m].template.manaUseType == 2) ? 1.0 : ((keySkill[m].template.manaUseType == 1) ? ((double)keySkill[m].manaUse * Char.myCharz().cMPFull / 100.0) : ((double)keySkill[m].manaUse)));
	...
	{
		if (Char.myCharz().isCreateDark || isCharging() || Char.myCharz().taskMaint.taskId <= 1 || ModFunc.isShowMenuChat || QuayTamBao.isTamBao || QuaNapTuan.isNapTuan)
		{
			return;
		}
		Char.myCharz().myskill = skill;
	...
	public override void update()
	{
		...
		try
		{
			...
			for (int num9 = 0; num9 < textTime.size(); num9++)
			{
				((ItemTime)textTime.elementAt(num9)).update();
			}
			updateChatVip();
		}
		catch (Exception)
	...
	public override void paint(mGraphics g)
	{
		...
		{
			Npc npc3 = (Npc)vNpc.elementAt(num7);
		...
		if (!isPaintOther)
		{
			if (ModFunc.isActiveNapTuan && QuaNapTuan.isNapTuan)
			{
				QuaNapTuan.paint(g);
				return;
			}
	...
	{
		int num = (int)(c.cHP * (double)hpBarW / c.cHPFull);
		int num2 = (int)c.cMP * mpBarW;
		int num3 = (int)(dHP * (double)hpBarW / c.cHPFull);
	...
	private void paintImageBar(mGraphics g, bool isLeft, Char c)
	{
		...
		if (c.charID == Char.myCharz().charID)
		{
			...
			{
				long hpLong = (long)(dHP * (double)hpBarW / c.cHPFull);
				long mpLong = (long)(dMP * (double)mpBarW / c.cMPFull);
				long currHPLong = (long)(c.cHP * (double)hpBarW / c.cHPFull);
				long num = (long)(c.cMP * (double)mpBarW / c.cMPFull);
				num2 = (int)hpLong;
				num3 = (int)mpLong;
				num4 = (int)currHPLong;
				num5 = (int)num;
			...
			{
				num2 = (int)(dHP * (double)hpBarW / c.cHPFull);
				num3 = (int)(dMP * (double)mpBarW / c.cMPFull);
				num4 = (int)(c.cHP * (double)hpBarW / c.cHPFull);
				num5 = (int)(c.cMP * (double)mpBarW / c.cMPFull);
			}
		}
		else
		{
			num2 = (int)(c.dHP * (double)hpBarW / c.cHPFull);
			num3 = c.perCentMp * mpBarW / 100;
			num4 = (int)(c.cHP * (double)hpBarW / c.cHPFull);
			num5 = c.perCentMp * mpBarW / 100;
		}
		...
		{
			g.setClip((int)(83 + hpBarW - num2), 5, num2, 10);
		}
		...
		{
			g.setClip((int)(83 + hpBarW - num4), 5, num4, 10);
		}
		...
		{
			g.setClip(83 + mpBarW - num3, 20, num3, 6);
		}
		...
		{
			g.setClip(83 + mpBarW - num5, 20, num5, 6);
		}
		g.drawImage(imgMP, 83, 20, 0);
		g.setClip(0, 0, GameCanvas.w, GameCanvas.h);
		if (Char.myCharz().cMP == 0.0 && GameCanvas.gameTick % 10 > 5)
		{
			g.setClip(83, 20, 2, 6);
			g.drawImage(imgMPLost, 83, 20, 0);
			g.setClip(0, 0, GameCanvas.w, GameCanvas.h);
	...
	private void paintSelectedSkill(mGraphics g)
	{
		...
		{
			if (QuaNapTuan.isNapTuan || QuayTamBao.isTamBao || ModFunc.isShowMenuChat || GameCanvas.currentDialog != null || ChatPopup.currChatPopup != null || GameCanvas.menu.showMenu || isPaintPopup() || GameCanvas.panel.isShow || Char.myCharz().taskMaint.taskId == 0 || ChatTextField.gI().isShow || GameCanvas.currentScreen == MoneyCharge.instance)
			{
				return;
			}
			long num2 = mSystem.currentTimeMillis() - lastUsePotion;
			int num3 = 0;
			if (num2 < 10000)
			{
			...
			else if (Char.myCharz().statusMe != 14)
			{
				...
				else
				{
					...
					g.drawImage((mScreen.keyTouch != 10) ? imgHP3 : imgHP4, xHP + 20, yHP + 20 - 3, mGraphics.HCENTER | mGraphics.VCENTER);
	...
	private void paintGamePad(mGraphics g)
	{
		if (isAnalog != 0 && Char.myCharz().statusMe != 14)
		{
			g.drawImage((mScreen.keyTouch != 5 && mScreen.keyMouse != 5) ? imgFire0 : imgFire1, xF + 20, yF + 14, mGraphics.HCENTER | mGraphics.VCENTER);
			gamePad.paint(g);
	...
	public static void paintHPBar_NEW(mGraphics g, int x, int y, Char c)
	{
		...
		num2 = (int)(c.cHP * (double)width / c.cHPFull);
		if (num2 <= 0)
		{
			num2 = 1;
		}
		...
		num3 = (int)(c.cMP * (double)width / c.cMPFull);
		if (num3 <= 0)
		{
			num3 = 1;
		}
	...
	private void paint_xp_bar(mGraphics g)
	{
		...
		int w = (int)(Char.myCharz().cLevelPercent * (double)GameCanvas.w / 10000.0);
		g.setColor(16777215);
